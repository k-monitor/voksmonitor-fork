...
export function ShareModal({ calculatorId, segments, isOpen, onClose }: ShareModalProps) {
  ...
  useEffect(() => {
    if (isOpen && !publicId) {
      ...
      shareSession(calculatorId)
        ...
        .catch(() => {
          setError(t("error-create-link"));
        })
        .finally(() => {
          setIsLoading(false);
        });
    }
  }, [isOpen, publicId, calculatorId, t]);
  ...
  const shareText = xHandle ? t("share-text-with-handle", { handle: xHandle }) : t("share-text");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()} role="document">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors" aria-label={t("close-aria-label")}>
        ...
        {shareUrl && !isLoading && !error && (
          <>
            <p className="text-slate-600 text-sm mb-4">{t("share-description")}</p>
              ...
              {hasClipboardAccess && (
                <div className="shrink-0">
                  <Button onClick={handleCopy} variant="outline" color="neutral" size="small">
                    ...
                    {isCopied ? t("copied-button") : t("copy-button")}
                  </Button>
                </div>
              )}
            </div>
