              ...
              <Card shadow="elevated" border corner="topLeft" className="bg-white h-full !border-gray-200">
                <div className="p-6 md:p-8 h-full flex flex-col">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-gray-600">
                    <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 font-semibold text-red-700">{t("featured-card.tags.questionnaire")}</span>
                    <span className="rounded-full bg-gray-100 px-2.5 py-1">{t("featured-card.tags.questions")}</span>
                    <span className="rounded-full bg-gray-100 px-2.5 py-1">{t("featured-card.tags.duration")}</span>
                  </div>
                  <h2 className="mt-4 font-display ko:font-display font-bold tracking-tight text-gray-700 text-2xl md:text-3xl">{t("featured-card.title")}</h2>
                  <p className="mt-2 text-gray-500">{t("featured-card.description")}</p>
                  <div className="grid mt-auto pt-4 md:pt-6">
                    <Link href="/vm/fovarosi-kozgyules/inventory" className="grid">
                      <Button variant="fill" color="primary">
                        {t("featured-card.start-button")}
                      </Button>
                    </Link>
                  </div>
                </div>
          ...
          <div className="mt-12">
            <div className="w-full max-w-6xl mx-auto">
              <div className="bg-gray-50 rounded-lg p-8 md:p-12">
                ...
                <p className="text-center text-gray-600 max-w-4xl mx-auto mb-12">{t("methodology-section.description")}</p>
